...
import 'package:omg/tin/stackedCardViewItemContent.dart';
    ...
    slideBackAnimation = AnimationController(
      ...
      ..addStatusListener((AnimationStatus status) {
        if (status == AnimationStatus.completed) {
          setState(() {
            dragStartPosition = Offset.zero; // 初期化
            dragCurrentPosition = Offset.zero; // 初期化
            slideBackStartPosition = Offset.zero; // 初期化
          });
        }
      });
      ...
      ..addStatusListener((AnimationStatus status) {
        if (status == AnimationStatus.completed) {
          setState(() {
            dragStartPosition = Offset.zero; // 初期化
            dragCurrentPosition = Offset.zero; // 初期化
            slideOutTween = Tween<Offset>(begin: Offset.zero, end: Offset.zero); // 初期化
  ...
  void _slideFromExternal() {
    switch (widget.card.direction) {
      ...
      case SlideDirection.Up:
        _slideUp();
        break;
    // SlideDirection の場合に `null` が可能か確認し、必要に応じて適切に処理する
      default:
        ...
        return Center(
          child: Transform(
            ...
            transform: Matrix4.identity()
              ..translate(containerOffset.dx, containerOffset.dy)
              ..rotateZ(_rotation(context.size! as Rect) * 4),
            child: GestureDetector(
              onPanStart: _onPanStart,
              onPanUpdate: _onPanUpdate,
              onPanEnd: _onPanEnd,
              child: SizedBox.expand(
                child: widget.card as Widget,
              ),
            ),
          ),
        );
